...
typedef enum _VC_FLAGS // from SH2, unsure if all are used in SH
{
  VC_USER_CAM_F = 0x1,
  VC_USER_WATCH_F = 0x2,
  VC_WARP_CAM_F = 0x4,
...
typedef struct _VC_WORK
{
  ...
  short geom_screen_dist_30; // related to GsSetProjection / GameSys_800BC728.gs_y_res_58A
  short field_32;
...
extern VC_CAMERA_INTINFO vcCameraInternalInfo; // debug camera mode
...
void vcSetEvCamRate_800401C0(short ev_cam_rate);
...
void vcSetRefPosAndSysRef2CamParam(VECTOR3 *ref_pos, SYS_W *sys_p, int for_f, int back_f, int right_f, int left_f, int up_f, int down_f);
void vcSetRefPosAndCamPosAngByPad(VECTOR3 *ref_pos, SYS_W *sys_p);

// vw_main
void vwInitViewInfo();
GsCOORDINATE2 *vwGetViewCoord_80048A90();
...
void vcChangeProjectionValue_80080D5C(short scr_y);
...
int Math_VectorMagnitude_80085B1C(int x, int y, int z);
